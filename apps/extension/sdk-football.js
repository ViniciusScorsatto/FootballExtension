(function initLiveMatchImpactSdk(globalScope) {
  ...
  function buildIdentityHeaders({ userId = "anonymous", plan = "free" } = {}) {
    return {
  ...
  function createSdkError(message, status = 0, data = null) {
  ...
  function createSdk({ baseUrl, getHeaders, requester }) {
    ...
    async function request(method, path, options = {}) {
      ...
      return resolvedRequester({
        method,
        url: `${normalizeBaseUrl(baseUrl)}${createPath(path, options.query)}`,
  ...
  globalScope.LMI_SDK = {
    ...
    createFootballSdk({ baseUrl, userId, plan, requester }) {
      return createSdk({
        baseUrl,
        requester,
    ...
    createRequesterBackedSdk({ baseUrl, getHeaders, requester }) {
      return createSdk({
        baseUrl,
        getHeaders,
        requester
